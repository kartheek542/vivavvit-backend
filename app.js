...
app.post("/register", async (request, response)=>{
  ...
  let placeholders = events.map((event) => '('+rollNo+','+event+')').join(',');
...
app.put("/coordinator/save", authenticate, async (request, response)=>{
  const { eventId,eventName,category,imageUrl,venue,dateTime,description} = request.body;
...
app.get("/coordinator/reports/filter",authenticate , async (request, response)=>{
